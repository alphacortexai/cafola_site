...
import {
  defaultSiteContent,
  type Article,
  type CustomPage as CustomPageType,
  type HomeImages,
  type MediaAsset,
  type Service,
...
export default function AdminEditor() {
  ...
  const [customPageDescriptionInput, setCustomPageDescriptionInput] =
    useState("");
  ...
  const loadCms = async () => {
    try {
      ...
      if (!response.ok)
        throw new Error(`Failed to load CMS (${response.status})`);
  ...
  const updateDraft = <K extends keyof SiteContent>(
    key: K,
    value: SiteContent[K]
  ) => {
  ...
  const updateHomeImage = <K extends keyof HomeImages>(
    key: K,
    value: HomeImages[K]
  ) => {
    setDraftCms(prev => ({
      ...prev,
      homeImages: {
        ...(prev.homeImages ?? defaultSiteContent.homeImages),
        [key]: value,
      },
    }));
  };
  ...
  const updateCaregiverCopy = (index: number, value: string) => {
    setDraftCms(prev => ({
      ...prev,
      caregiversCopy: prev.caregiversCopy.map((line, i) =>
        i === index ? value : line
      ),
    }));
  };
  ...
  const addMediaAsset = (asset: MediaAsset) => {
    setDraftCms(prev => ({
      ...prev,
      mediaLibrary: [asset, ...(prev.mediaLibrary ?? [])],
    }));
  };
  ...
  const deleteMediaAsset = (assetId: string) => {
    setDraftCms(prev => ({
      ...prev,
      mediaLibrary: (prev.mediaLibrary ?? []).filter(
  ...
  const onLogoSelected = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setStatus(
        "Uploaded company logo. Publish changes to apply it site-wide."
      );
    } catch (error) {
  ...
  const updateAboutUs = <K extends keyof SiteContent["aboutUs"]>(
    key: K,
    value: SiteContent["aboutUs"][K]
  ) => {
    setDraftCms(prev => ({
      ...prev,
    ...
    setDraftCms(prev => ({
      ...
      services: prev.services.map((service, i) =>
        i === serviceIndex
          ? {
              ...service,
              details: [...(service.details ?? []), emptyServiceDetail],
            }
          : service
      ),
    }));
    ...
    setDraftCms(prev => ({
      ...
      services: prev.services.map((service, i) =>
        i === serviceIndex
          ? {
              ...service,
              details: (service.details ?? []).filter(
                (_, j) => j !== detailIndex
              ),
            }
          : service
  ...
  const addServicePage = (page?: Partial<ServicePage>) => {
    ...
    setDraftCms(prev => ({
      ...prev,
      servicePages: [...(prev.servicePages ?? []), nextPage],
    }));
    ...
    setDraftCms(prev => {
      ...
      return {
        ...
        services: prev.services.map(service => ({
          ...service,
          details: (service.details ?? []).map(detail =>
            detail.linkPageSlug === removed?.slug
              ? { ...detail, linkPageSlug: "" }
              : detail
          ),
        })),
    ...
    setDraftCms(prev => ({
      ...
      servicePages: (prev.servicePages ?? []).map((page, i) =>
        i === pageIndex
          ? {
              ...page,
              sections: [...(page.sections ?? []), emptyServicePageSection],
            }
          : page
      ),
    }));
  ...
  const deleteServicePageSection = (
    pageIndex: number,
    sectionIndex: number
  ) => {
    setDraftCms(prev => ({
  ...
  const getUniqueServicePageSlug = (title: string) => {
    const base =
      title
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-|-$/g, "") || "service-page";
    const existing = new Set(
      (draftCms.servicePages ?? []).map(page => page.slug)
    );
    let slug = base;
    let count = 2;
    ...
    <div className="rounded border border-slate-800 bg-slate-950/80 p-3 space-y-3">
      ...
      {(draftCms.mediaLibrary ?? []).length > 0 ? (
        <div className="grid grid-cols-2 gap-2">
          {(draftCms.mediaLibrary ?? []).map(asset => (
            ...
            >
              ...
              <span className="block truncate px-2 py-2 text-xs text-slate-300">
                {asset.name}
              </span>
            </button>
          ))}
        ...
        <p className="text-sm text-slate-400">
          Upload media below to make it available here.
        </p>
      )}
    </div>
  );
  ...
  const renderImagePicker = (
    ...
    const imageAssets = (draftCms.mediaLibrary ?? []).filter(
    ...
    return (
      <div className="rounded border border-slate-800 bg-slate-950/80 p-3 space-y-3">
        ...
        {imageAssets.length > 0 ? (
          ...
          <p className="text-sm text-slate-400">
            Upload images in the Media section to make them available here.
          </p>
        )}
      </div>
    );
  ...
  const renderLinkedPageHeroEditor = (
    page: ServicePage,
    pageIndex: number,
    ...
    <div className="grid gap-3 rounded border border-slate-800 bg-slate-900/70 p-3">
      <div>
        <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">
          Linked page first image
        </p>
        <p className="text-xs text-slate-500">
          This replaces the default image shown at the top of the linked page.
        </p>
      </div>
      <input
        value={page.heroImageUrl ?? ""}
        onChange={e =>
          updateServicePage(pageIndex, { heroImageUrl: e.target.value })
        }
        placeholder="First image or video URL"
        className={inputClassName}
      />
      {renderMediaPicker(page.heroImageUrl, asset =>
        updateServicePage(pageIndex, {
          heroImageUrl: asset.url,
          heroMediaType: asset.type,
        })
      )}
      <select
        ...
        onChange={e =>
          updateServicePage(pageIndex, {
            heroMediaType: e.target.value as ServicePage["heroMediaType"],
          })
        }
        className={inputClassName}
      >
        <option value="image">First media is image</option>
        <option value="video">First media is video</option>
      </select>
    </div>
  );
  ...
  const renderServiceDetailsEditor = (
    service: Service,
    serviceIndex: number,
    inputClassName: string
  ) => (
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400">
            These appear under "What We Offer" on this service page.
          </p>
        </div>
        <Button
          type="button"
      ...
      {(service.details ?? []).length > 0 ? (
        <div className="space-y-3">
          {(service.details ?? []).map((detail, detailIndex) => {
            const linkedPageIndex = (draftCms.servicePages ?? []).findIndex(
              page => page.slug === detail.linkPageSlug
            );
            const linkedPage =
              linkedPageIndex >= 0
                ? (draftCms.servicePages ?? [])[linkedPageIndex]
                : undefined;

            return (
              <div
                key={`${detail.title}-${detailIndex}`}
                className="rounded border border-slate-800 bg-slate-950 p-3 space-y-3"
              >
                ...
                <input
                  value={detail.title}
                  onChange={e =>
                    updateServiceDetail(serviceIndex, detailIndex, {
                ...
                <label className="flex items-start gap-3 rounded border border-slate-800 bg-slate-900/70 p-3 text-sm text-slate-300">
                  <input
                    ...
                    onChange={e =>
                      updateServiceDetail(serviceIndex, detailIndex, {
                        inlineLinkedPage: e.target.checked,
                      })
                    }
                    className="mt-1"
                  />
                  <span>
                    <span className="block font-semibold text-slate-100">
                      Show linked page content inline
                    </span>
                    <span className="block text-xs text-slate-400">
                      When enabled, the linked page details display before “What
                      We Offer” instead of this block appearing as a card.
                    </span>
                  </span>
                </label>
                <div className="grid gap-2">
                  ...
                  <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
                    ...
                    <Button
                      ...
                      className="text-white"
                      onClick={() => {
                        const slug = getUniqueServicePageSlug(
                          detail.title || service.title || "service-page"
                        );
                        addServicePage({
                          slug,
        ...
        {(draftCms.servicePages ?? []).map((page, pageIndex) => (
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <div>
                <h3 className="font-semibold">
                  {page.title || page.slug || `Service page ${pageIndex + 1}`}
                </h3>
                <p className="text-xs text-slate-500">
                  /services/&lt;service&gt;/{page.slug || "slug"}
                </p>
              </div>
            ...
            <input
              value={page.title}
              onChange={e =>
                updateServicePage(pageIndex, { title: e.target.value })
              }
              placeholder="Title"
              className="w-full px-3 py-2 bg-slate-900 border border-slate-700"
            />
            <textarea
              ...
              onChange={e =>
                updateServicePage(pageIndex, { description: e.target.value })
              }
            ...
            <textarea
              value={page.content ?? ""}
              onChange={e =>
                updateServicePage(pageIndex, { content: e.target.value })
              }
            ...
            <div className="space-y-3 rounded border border-slate-800 bg-slate-900/50 p-4">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    Add images and choose where each image sits on the page.
                  </p>
                </div>
                <Button
                  type="button"
                    ...
                    >
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <Button
                          ...
                          className="text-white"
                          onClick={() =>
                            deleteServicePageSection(pageIndex, sectionIndex)
                          }
                        >
                          Remove
                        </Button>
                      ...
                      <select
                        ...
                        onChange={e =>
                          updateServicePageSection(pageIndex, sectionIndex, {
                            mediaType: e.target
                              .value as ServicePageSection["mediaType"],
                          })
                        }
                        className="w-full px-3 py-2 bg-slate-900 border border-slate-700"
                      >
                        ...
                        onChange={e =>
                          updateServicePageSection(pageIndex, sectionIndex, {
                            imagePosition: e.target
                              .value as ServicePageSection["imagePosition"],
                          })
                        }
                        className="w-full px-3 py-2 bg-slate-900 border border-slate-700"
                      >
        ...
        <main className="container mx-auto px-4 py-10">
          <section className="max-w-2xl bg-slate-900 border border-slate-800 p-6 space-y-4">
            ...
            <p className="text-slate-300 text-sm leading-relaxed">
              You must sign in with Gmail before you can use the full site
              editor.
            </p>
        ...
        <div className="container mx-auto px-4 py-4 flex flex-col gap-4 md:flex-row md:justify-between md:items-center">
          <div>
            ...
            <p className="text-slate-400 text-sm">
              Edit components, page content, links, and preview the site live.
            </p>
          </div>
        ...
        <section className="bg-slate-900 border border-slate-800 p-6">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-400">
                The preview reflects your current draft. Publish to save it to
                Firebase and make it the live CMS content.
              </p>
            </div>
            <div className="flex flex-wrap gap-3">
              <Button
              ...
              <EditorSection title="Edit About Us">
                <div className="grid gap-3">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-slate-950 border border-slate-700"
                  />
                  {renderImagePicker(draftCms.aboutUs.imageUrl, asset =>
                    updateAboutUs("imageUrl", asset.url)
                  )}
                  <input
                  ...
                  return (
                    ...
                    >
                      ...
                      <input
                        ...
                        onChange={e =>
                          updateService(index, { imageUrl: e.target.value })
                        }
                        placeholder="Service detail image URL"
                        className="w-full px-3 py-2 bg-slate-950 border border-slate-700"
                      />
                      {renderImagePicker(service.imageUrl, asset =>
                        updateService(index, { imageUrl: asset.url })
                      )}
                      <input
                        value={service.imageAlt ?? ""}
                        onChange={e =>
                          updateService(index, { imageAlt: e.target.value })
                        }
                  ...
                  return (
                    ...
                    >
                      ...
                      <select
                        ...
                        onChange={e =>
                          updateServicePage(pageIndex, {
                            heroMediaType: e.target
                              .value as ServicePage["heroMediaType"],
                          })
                        }
                        className="w-full px-3 py-2 bg-slate-950 border border-slate-700"
                      >
                      ...
                      <div className="space-y-3 rounded border border-slate-800 bg-slate-900/50 p-4">
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                          <div>
                            <h4 className="font-semibold">Page sections</h4>
                            <p className="text-xs text-slate-400">
                              Add titles, text, images, and image placement for
                              this page.
                            </p>
                          </div>
                          <Button
                  ...
                  return (
                    <div key={`${article.slug}-${index}`} className="space-y-3">
                      ...
                      <input
                        ...
                        onChange={e =>
                          updateArticle(index, { section: e.target.value })
                        }
                        placeholder="Section"
                        className="w-full px-3 py-2 bg-slate-950 border border-slate-700"
                      />
                      <input
                        ...
                        onChange={e =>
                          updateArticle(index, { imageUrl: e.target.value })
                        }
                        placeholder="Image URL"
                        className="w-full px-3 py-2 bg-slate-950 border border-slate-700"
                      />
                      {renderImagePicker(article.imageUrl, asset =>
                        updateArticle(index, { imageUrl: asset.url })
                      )}
                      <textarea
                      ...
                      <textarea
                        ...
                        onChange={e =>
                          updateArticle(index, { content: e.target.value })
                        }
                        placeholder="Content"
                        className="w-full px-3 py-2 bg-slate-950 border border-slate-700 min-h-[100px]"
                      />
                  ...
                  return (
                    <div key={`${page.slug}-${index}`} className="space-y-3">
                      <input
                        value={page.title}
                        onChange={e =>
                          updateCustomPage(index, { title: e.target.value })
                        }
                        placeholder="Title"
                        className="w-full px-3 py-2 bg-slate-950 border border-slate-700"
                      />
                      <input
                        value={page.slug}
                        onChange={e =>
                          updateCustomPage(index, { slug: e.target.value })
                        }
                        placeholder="Slug"
                        className="w-full px-3 py-2 bg-slate-950 border border-slate-700"
                      />
                      ...
                      <input
                        value={page.imageUrl ?? ""}
                        onChange={e =>
                          updateCustomPage(index, { imageUrl: e.target.value })
                        }
                        placeholder="Page image URL"
                        className="w-full px-3 py-2 bg-slate-950 border border-slate-700"
                      />
                      {renderImagePicker(page.imageUrl, asset =>
                        updateCustomPage(index, { imageUrl: asset.url })
                      )}
                      <input
                        value={page.imageAlt ?? ""}
                        onChange={e =>
                          updateCustomPage(index, { imageAlt: e.target.value })
                        }
                      ...
                      <textarea
                        value={page.content}
                        onChange={e =>
                          updateCustomPage(index, { content: e.target.value })
                        }
                        placeholder="Content"
                ...
                <EditorSection title="Site settings">
                  <div className="grid gap-3">
                    ...
                    <input
                      ...
                      onChange={e =>
                        updateDraft("companyDescriptor", e.target.value)
                      }
                    ...
                    <div className="space-y-3 rounded border border-slate-800 bg-slate-950 p-4">
                      <div>
                        ...
                        <p className="text-sm text-slate-400">
                          Upload or choose the logo used in the fixed navigation
                          bar and brand areas.
                        </p>
                      </div>
                      {draftCms.logoUrl ? (
                        <img
                          src={draftCms.logoUrl}
                          alt="Current company logo"
                          className="h-20 w-20 rounded bg-white object-contain p-2"
                        />
                      ) : null}
                      <input
                      ...
                      />
                      {renderImagePicker(draftCms.logoUrl, asset =>
                        updateDraft("logoUrl", asset.url)
                      )}
                    </div>
                    <input
                    ...
                    <textarea
                      ...
                      onChange={e =>
                        updateDraft("heroSubheading", e.target.value)
                      }
                    ...
                    <input
                      ...
                      onChange={e =>
                        updateDraft("servicesHeading", e.target.value)
                      }
                    ...
                    <textarea
                      ...
                      onChange={e =>
                        updateDraft("servicesIntro", e.target.value)
                      }
                    ...
                    <div className="space-y-3 rounded border border-slate-800 bg-slate-950 p-4">
                      ...
                      <input
                        ...
                        onChange={e =>
                          updateDraft("caregiversHeading", e.target.value)
                        }
                      ...
                      {draftCms.caregiversCopy.map((line, index) => (
                        <div
                          key={`caregiver-copy-${index}`}
                          className="flex items-start gap-3"
                        >
                          <textarea
                            value={line}
                            onChange={e =>
                              updateCaregiverCopy(index, e.target.value)
                            }
                          ...
                          <Button
                            ...
                            onClick={() =>
                              updateDraft(
                                "caregiversCopy",
                                draftCms.caregiversCopy.filter(
                                  (_, i) => i !== index
                                )
                              )
                            }
                      ...
                      <div className="flex flex-wrap gap-3">
                        <Button
                          ...
                          className="bg-teal hover:bg-teal/90"
                          onClick={() =>
                            updateDraft("caregiversCopy", [
                              ...draftCms.caregiversCopy,
                              "",
                            ])
                          }
                        >
                      ...
                      <input
                        ...
                        onChange={e =>
                          updateDraft("caregiversButtonText", e.target.value)
                        }
                    ...
                    <div className="space-y-3 rounded border border-slate-800 bg-slate-950 p-4">
                      ...
                      <input
                        ...
                        onChange={e =>
                          updateDraft("resourcesHeading", e.target.value)
                        }
                      ...
                      <textarea
                        ...
                        onChange={e =>
                          updateDraft("resourcesIntro", e.target.value)
                        }
                    ...
                    <div className="space-y-3 rounded border border-slate-800 bg-slate-950 p-4">
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 bg-slate-900 border border-slate-700"
                      />
                      {renderImagePicker(draftCms.homeImages?.hero, asset =>
                        updateHomeImage("hero", asset.url)
                      )}
                      <input
                        ...
                        className="w-full px-3 py-2 bg-slate-900 border border-slate-700"
                      />
                      {renderImagePicker(draftCms.homeImages?.story, asset =>
                        updateHomeImage("story", asset.url)
                      )}
                      {(
                        draftCms.homeImages?.caregivers ??
                        defaultSiteContent.homeImages.caregivers
                      ).map((url, index) => (
                        <div
                          key={`caregiver-image-${index}`}
                          className="space-y-2 rounded border border-slate-800 bg-slate-900/50 p-3"
                        >
                          <input
                            value={url}
                            onChange={e =>
                              updateCaregiverImage(index, e.target.value)
                            }
                            placeholder={`Caregiver image ${index + 1} URL`}
                            className="w-full px-3 py-2 bg-slate-900 border border-slate-700"
                          />
                          {renderImagePicker(url, asset =>
                            updateCaregiverImage(index, asset.url)
                          )}
                        </div>
                      ))}
                    </div>
                      ...
                      >
                        ...
                        <Button
                          ...
                          onClick={() =>
                            updateDraft(
                              "navItems",
                              draftCms.navItems.filter((_, i) => i !== index)
                            )
                          }
                        >
                          Remove
                ...
                <EditorSection title="Footer & About">
                  <div className="grid gap-3">
                    ...
                    <textarea
                      value={draftCms.aboutUs.description}
                      onChange={e =>
                        updateAboutUs("description", e.target.value)
                      }
                      placeholder="About description"
                      className="w-full px-3 py-2 bg-slate-950 border border-slate-700 min-h-[80px]"
                    />
                    <div className="grid gap-3 sm:grid-cols-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          updateDraft("footerLinks", [
                            ...draftCms.footerLinks,
                            footerLinkInput.trim(),
                          ]);
                          setFooterLinkInput("");
                        }}
                        className="bg-teal hover:bg-teal/90"
                    ...
                    <div className="grid gap-3">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          updateDraft("footerAddress", [
                            ...draftCms.footerAddress,
                            addressInput.trim(),
                          ]);
                          setAddressInput("");
                        }}
          ...
          <div className="space-y-6 xl:max-h-[calc(100vh-160px)] xl:overflow-y-auto xl:pr-2">
            <section className="bg-white border border-slate-200 p-6 rounded-md overflow-hidden space-y-6">
              <div className="mb-4 flex items-center justify-between gap-4">
                <div>
                  <h2 className="text-xl font-serif text-slate-950">
                    Live preview
                  </h2>
                  <p className="text-sm text-slate-600">
                    Preview the current draft across pages and custom content.
                  </p>
                </div>
                <Button
                  type="button"
              ...
              <div className="space-y-5 rounded-md border border-slate-200 bg-slate-50 p-4">
                <div>
                  ...
                  <p className="text-sm text-slate-600">
                    Switch between the homepage, About, Resources, service
                    pages, linked service pages, articles, or custom pages.
                  </p>
                </div>
                <div className="grid gap-3 sm:grid-cols-2">
                          ...
                          ((draftCms.servicePages ?? []).length > 0 ? (
                            (draftCms.servicePages ?? []).map(page => (
                              <option key={page.slug} value={page.slug}>
                                {page.title ||
                                  page.slug ||
                                  "Untitled service page"}
                              </option>
                            ))
                          ) : (
                            <option value="">
                              No service linked pages available
                            </option>
                          ))}
                        {previewPage === "custom" &&
                          (draftCms.customPages.length > 0 ? (
              ...
              <div className="space-y-5 rounded-md border border-slate-200 bg-slate-50 p-4">
                <div>
                  ...
                  <p className="text-sm text-slate-600">
                    Create or update custom page content that appears at{" "}
                ...
                <div className="grid gap-3">
                  ...
                  <input
                    ...
                    onChange={e =>
                      setCustomPageDescriptionInput(e.target.value)
                    }
